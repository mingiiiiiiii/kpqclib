...
/**
 * \fn void rbc_67_vec_set_random(random_source*, rbc_67_vec v, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_67_vec_set_random(random_source* ctx, rbc_67_vec v, uint32_t size) {
...
/**
 * \fn void rbc_67_vec_set_random_full_rank(random_source* ctx, rbc_67_vec o, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_67_vec_set_random_full_rank(random_source* ctx, rbc_67_vec o, uint32_t size) {
  int32_t rank_max = RBC_67_FIELD_M < size ? RBC_67_FIELD_M : size;
  int32_t rank = -1;

  while(rank != rank_max) {
    rbc_67_vec_set_random(ctx, o, size);
...
/**
 * \fn void rbc_67_vec_set_random_full_rank_with_one(random_source* ctx, rbc_67_vec o, uint32_t size) {
...
/**
 * \fn void rbc_67_vec_set_random_from_support(random_source* o, rbc_67_vec o, uint32_t size, const rbc_67_vec support, uint32_t support_size)
...
/**
 * \fn void rbc_67_vec_set_random_pair_from_support(random_source* o, rbc_67_vec o1, rbc_67_vec o2, uint32_t size, const rbc_67_vec support, uint32_t support_size)
 ...
 * 
 * \param[out] ctx random source
 * \param[out] o1 rbc_67_vec
 * \param[out] o2 rbc_67_vec
  ...
  if (copy_flag) {
    ...
    while(i != support_size) {
      ...
      if(position < size * (256 / size)) {
        if(random1[j + 1] & 0x1) {
          if(rbc_67_elt_is_zero(o1[position % size])) {
            rbc_67_elt_set(o1[position % size], support[i]);
            i++;
          }
        } 
        else {
          if(rbc_67_elt_is_zero(o2[position % size])) {
            rbc_67_elt_set(o2[position % size], support[i]);
            i++;
          }
        }
      }
    ...
    for(i = 0 ; i < size ; ++i) {
      if(rbc_67_elt_is_zero(o2[i])) {
        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_67_elt_add(o2[i], support[j], o2[i]);
          }
...
uint32_t rbc_67_vec_gauss(rbc_67_vec v, uint32_t size, rbc_67_vec *other_matrices, uint32_t nMatrices) {
  ...
  //For each column
  for(size_t i = 0 ; i < RBC_67_FIELD_M ; i++) {
    int64_t mask;
    size_t column = RBC_67_FIELD_M - i - 1;
    uint32_t row = min(dimension, size-1);

    //For each line below
    for(size_t j=0 ; j < size ; j++) {
        rbc_67_elt_add(sum, v[row], v[j]);
        mask = rbc_67_elt_get_coefficient_vartime(sum, column);
        rbc_67_elt_set_mask1(tmp, v[j], zero, mask);
        if(j > row) {
          rbc_67_elt_add(v[row], v[row], tmp);
          for(size_t k=0 ; k<nMatrices ; k++) {
            rbc_67_elt_set_mask1(tmp, other_matrices[k][j], zero, mask);
            rbc_67_elt_add(other_matrices[k][row], other_matrices[k][row], tmp);
          }
        }
        else {
          rbc_67_elt_add(dummy, v[row], tmp);
          for(size_t k=0 ; k<nMatrices ; k++) {
            rbc_67_elt_add(dummy, other_matrices[k][row], tmp);
          }
        }
    }
    ...
    for(size_t j=0 ; j < size ; j++) {
      if(j == row) continue;
      mask = rbc_67_elt_get_coefficient_vartime(v[j], column);
      rbc_67_elt_set_mask1(tmp, v[row], zero, mask);
      if(dimension < size) {
        rbc_67_elt_add(v[j], v[j], tmp);
        for(size_t k=0 ; k<nMatrices ; k++) {
          rbc_67_elt_set_mask1(tmp, other_matrices[k][row], zero, mask);
          rbc_67_elt_add(other_matrices[k][j], other_matrices[k][j], tmp);
        }
      }
      else {
        rbc_67_elt_add(dummy, v[j], tmp);
        for(size_t k=0 ; k<nMatrices ; k++) {
          rbc_67_elt_add(dummy, other_matrices[k][j], tmp);
        }
      }
    }

    dimension += rbc_67_elt_get_coefficient_vartime(v[row], column);
  }
...
/**
 * \fn uint32_t rbc_67_vec_gauss_vartime(rbc_67_vec v, uint32_t size, rbc_67_vec *other_matrices, uint32_t nMatrices)
 * \brief This function transform a vector of finite field elements to its row echelon form and returns its rank.
 *
 * Replicates linear operations on the nMatrices matrices indexed by other_matrices.
 *
...
uint32_t rbc_67_vec_gauss_vartime(rbc_67_vec v, uint32_t size, rbc_67_vec *other_matrices, uint32_t nMatrices) {
  ...
  target_deg = RBC_67_FIELD_M;

  for(size_t i = 0 ; i < size ; i++) {
    // Take the pivot as the greatest coordinate of v
    pivot = i;
    current_deg = rbc_67_elt_get_degree(v[pivot]);
    ...
    for(size_t j = pivot + 1 ; j < size ; j++) {
      ...
      int j_deg = rbc_67_elt_get_degree(v[j]);
    ...
    rbc_67_elt tmp;
    rbc_67_elt_set(tmp, v[i]);
    rbc_67_elt_set(v[i], v[pivot]);
    rbc_67_elt_set(v[pivot], tmp);

    for(size_t k=0 ; k<nMatrices ; k++) {
      rbc_67_elt_set(tmp, other_matrices[k][i]);
      rbc_67_elt_set(other_matrices[k][i], other_matrices[k][pivot]);
      rbc_67_elt_set(other_matrices[k][pivot], tmp);
    }
    degree = rbc_67_elt_get_degree(v[i]);

    // Add matrix columns whenever necessary
    for(size_t j = i + 1; j < size; j++) {
      if(!(rbc_67_elt_is_zero(v[j]) || rbc_67_elt_get_degree(v[j]) < degree)) {
        rbc_67_elt_add(v[j], v[j], v[i]);
        for(size_t k=0 ; k<nMatrices ; k++) {
          rbc_67_elt_add(other_matrices[k][j], other_matrices[k][j], other_matrices[k][i]);
        }
      }
    }
  }
...
uint32_t rbc_67_vec_get_rank(const rbc_67_vec v, uint32_t size) {
  ...
  dimension = rbc_67_vec_gauss(copy, size, NULL, 0);
...
/**
 * \fn uint32_t rbc_67_vec_get_rank_vartime(const rbc_67_vec v, uint32_t size)
 * \brief This function computes the rank of a vector of finite field elements.
 *
 * \param[in] v rbc_67_vec
 * \param[in] size Size of the vector
...
uint32_t rbc_67_vec_get_rank_vartime(const rbc_67_vec v, uint32_t size) {
  ...
  rbc_67_vec_init(&copy, size);
  rbc_67_vec_set(copy, v, size);
  dimension = rbc_67_vec_gauss_vartime(copy, size, NULL, 0);
  rbc_67_vec_clear(copy);
...
/**
 * \fn uint32_t rbc_67_vec_echelonize(rbc_67_vec v, uint32_t size)
 ...
 * \return Rank of the vector <b>v</b>
 */
uint32_t rbc_67_vec_echelonize(rbc_67_vec v, uint32_t size) {
  uint32_t degree;
  uint32_t dimension = rbc_67_vec_gauss(v, size, NULL, 0);

  for(int32_t i = dimension - 1 ; i >= 1 ; i--) {
    degree = rbc_67_elt_get_degree(v[i]);

    for(int32_t j = i - 1 ; j >= 0 ; j--) {
      if(rbc_67_elt_get_coefficient_vartime(v[j], degree) == 1) {
        rbc_67_elt_add(v[j], v[j], v[i]);
      }
    }
  }
...
void rbc_67_vec_to_string(uint8_t* str, const rbc_67_vec v, uint32_t size) {
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      uint8_t bit = rbc_67_elt_get_coefficient_vartime(v[i], RBC_67_FIELD_M - j);
      *(str + index) |= (bit << k % 8);
      k++;
      if(k % 8 == 0) index++;
    }
