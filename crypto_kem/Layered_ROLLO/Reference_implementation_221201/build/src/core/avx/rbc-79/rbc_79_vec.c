...
uint32_t rbc_79_vec_gauss(rbc_79_vec v, uint32_t size, rbc_79_vec *other_matrices, uint32_t nMatrices) {
  ...
  for(size_t i = 0 ; i < RBC_79_FIELD_M ; i++) {
    int64_t mask;
    size_t column = RBC_79_FIELD_M - i - 1;
    uint32_t row = min(dimension, size-1);

    //For each line below
    for(size_t j=0 ; j < size ; j++) {
        rbc_79_elt_add(sum, v[row], v[j]);
        mask = rbc_79_elt_get_coefficient_vartime(sum, column);
        rbc_79_elt_set_mask1(tmp, v[j], zero, mask);
        if(j > row) {
          rbc_79_elt_add(v[row], v[row], tmp);
          for(size_t k=0 ; k<nMatrices ; k++) {
            rbc_79_elt_set_mask1(tmp, other_matrices[k][j], zero, mask);
            rbc_79_elt_add(other_matrices[k][row], other_matrices[k][row], tmp);
          }
        }
        else {
          rbc_79_elt_add(dummy, v[row], tmp);
          for(size_t k=0 ; k<nMatrices ; k++) {
            rbc_79_elt_add(dummy, other_matrices[k][row], tmp);
          }
        }
    }
    ...
    for(size_t j=0 ; j < size ; j++) {
      if(j == row) continue;
      mask = rbc_79_elt_get_coefficient_vartime(v[j], column);
      rbc_79_elt_set_mask1(tmp, v[row], zero, mask);
      if(dimension < size) {
        rbc_79_elt_add(v[j], v[j], tmp);
        for(size_t k=0 ; k<nMatrices ; k++) {
          rbc_79_elt_set_mask1(tmp, other_matrices[k][row], zero, mask);
          rbc_79_elt_add(other_matrices[k][j], other_matrices[k][j], tmp);
        }
      }
      else {
        rbc_79_elt_add(dummy, v[j], tmp);
        for(size_t k=0 ; k<nMatrices ; k++) {
          rbc_79_elt_add(dummy, other_matrices[k][j], tmp);
        }
      }
    }

    dimension += rbc_79_elt_get_coefficient_vartime(v[row], column);
  }
...
/**
 * \fn uint32_t rbc_79_vec_gauss_vartime(rbc_79_vec v, uint32_t size, rbc_79_vec *other_matrices, uint32_t nMatrices)
 * \brief This function transform a vector of finite field elements to its row echelon form and returns its rank.
 *
 * Replicates linear operations on the nMatrices matrices indexed by other_matrices.
 *
...
uint32_t rbc_79_vec_gauss_vartime(rbc_79_vec v, uint32_t size, rbc_79_vec *other_matrices, uint32_t nMatrices) {
  ...
  target_deg = RBC_79_FIELD_M;

  for(size_t i = 0 ; i < size ; i++) {
    // Take the pivot as the greatest coordinate of v
    pivot = i;
    current_deg = rbc_79_elt_get_degree(v[pivot]);
    ...
    for(size_t j = pivot + 1 ; j < size ; j++) {
      ...
      int j_deg = rbc_79_elt_get_degree(v[j]);
    ...
    rbc_79_elt tmp;
    rbc_79_elt_set(tmp, v[i]);
    rbc_79_elt_set(v[i], v[pivot]);
    rbc_79_elt_set(v[pivot], tmp);

    for(size_t k=0 ; k<nMatrices ; k++) {
      rbc_79_elt_set(tmp, other_matrices[k][i]);
      rbc_79_elt_set(other_matrices[k][i], other_matrices[k][pivot]);
      rbc_79_elt_set(other_matrices[k][pivot], tmp);
    }
    degree = rbc_79_elt_get_degree(v[i]);

    // Add matrix columns whenever necessary
    for(size_t j = i + 1; j < size; j++) {
      if(!(rbc_79_elt_is_zero(v[j]) || rbc_79_elt_get_degree(v[j]) < degree)) {
        rbc_79_elt_add(v[j], v[j], v[i]);
        for(size_t k=0 ; k<nMatrices ; k++) {
          rbc_79_elt_add(other_matrices[k][j], other_matrices[k][j], other_matrices[k][i]);
        }
      }
    }
  }
...
uint32_t rbc_79_vec_get_rank(const rbc_79_vec v, uint32_t size) {
  ...
  dimension = rbc_79_vec_gauss(copy, size, NULL, 0);
...
/**
 * \fn uint32_t rbc_79_vec_get_rank_vartime(const rbc_79_vec v, uint32_t size)
 * \brief This function computes the rank of a vector of finite field elements.
 *
 * \param[in] v rbc_79_vec
 * \param[in] size Size of the vector
...
uint32_t rbc_79_vec_get_rank_vartime(const rbc_79_vec v, uint32_t size) {
  ...
  rbc_79_vec_init(&copy, size);
  rbc_79_vec_set(copy, v, size);
  dimension = rbc_79_vec_gauss_vartime(copy, size, NULL, 0);
  rbc_79_vec_clear(copy);
...
uint32_t rbc_79_vec_echelonize(rbc_79_vec v, uint32_t size) {
  uint32_t degree;
  uint32_t dimension = rbc_79_vec_gauss(v, size, NULL, 0);

  for(int32_t i = dimension - 1 ; i >= 1 ; i--) {
    degree = rbc_79_elt_get_degree(v[i]);

    for(int32_t j = i - 1 ; j >= 0 ; j--) {
      if(rbc_79_elt_get_coefficient_vartime(v[j], degree) == 1) {
        rbc_79_elt_add(v[j], v[j], v[i]);
      }
    }
  }
...
void rbc_79_vec_to_string(uint8_t* str, const rbc_79_vec v, uint32_t size) {
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      uint8_t bit = rbc_79_elt_get_coefficient_vartime(v[i], RBC_79_FIELD_M - j);
      *(str + index) |= (bit << k % 8);
      k++;
      if(k % 8 == 0) index++;
    }
