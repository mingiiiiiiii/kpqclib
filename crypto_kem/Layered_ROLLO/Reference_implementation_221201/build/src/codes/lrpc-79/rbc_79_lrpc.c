...
unsigned int rbc_79_lrpc_RSR(rbc_79_vspace E, const uint32_t E_expected_dim, const rbc_79_vspace F, const uint32_t F_dim, const rbc_79_vec ec, const uint32_t ec_size) {
  ...
  rbc_79_vec_gauss(S, ec_size, NULL, 0);
  ...
  E_dim = rbc_79_vec_gauss(E, E_expected_dim * F_dim, NULL, 0);
...
/**
 * @fn         unsigned int rbc_79_lrpc_RSR_vartime(rbc_79_vspace E, const uint32_t E_expected_dim, const rbc_79_vspace F, const uint32_t F_dim, const rbc_79_vec ec, const uint32_t ec_size)
 *
 * @brief      This function implements the rank_support_recover using
...
unsigned int rbc_79_lrpc_RSR_vartime(rbc_79_vspace E, const uint32_t E_expected_dim, const rbc_79_vspace F, const uint32_t F_dim, const rbc_79_vec ec, const uint32_t ec_size) {
  ...
  rbc_79_vspace_init(&S, ec_size);
  rbc_79_vspace_set(S, ec, ec_size);
  S_dim = rbc_79_vec_gauss_vartime(S, ec_size, NULL, 0);
  ...
  // Compute the spaces Si = F[i]^(-1) * S (line 10)
  for(size_t i = 0 ; i < F_dim ; ++i) {
    rbc_79_vspace_init(&(Si[i]), S_dim);
    rbc_79_elt_inv(Fi_inv, F[i]);
    rbc_79_vec_scalar_mul(Si[i], S, Fi_inv, S_dim);
  }

  //Intersect every Si,i+1
  //This supposes d >= 4
  E_dim = rbc_79_vspace_intersection_vartime(E, Si[0], Si[1], S_dim, S_dim);

  for(size_t i = 0 ; i < F_dim ; i++) {
    if(E_dim == E_expected_dim) break;
    E_dim = rbc_79_vspace_intersection_vartime(E, E, Si[i], E_dim, S_dim);
  }

  rbc_79_vec_echelonize(E, E_dim);
