/** 
 * \file rbc_67_lrpc.cpp
 * \brief Implementation of rbc_67_lrpc.h
 */
...
/**
 * @fn         unsigned int rbc_67_lrpc_RSR(rbc_67_vspace E, const uint32_t E_expected_dim, const rbc_67_vspace F, const uint32_t F_dim, const rbc_67_vec ec, const uint32_t ec_size)
 *
...
unsigned int rbc_67_lrpc_RSR(rbc_67_vspace E, const uint32_t E_expected_dim, const rbc_67_vspace F, const uint32_t F_dim, const rbc_67_vec ec, const uint32_t ec_size) {
  ...
  rbc_67_vspace S;
  rbc_67_vspace Si[F_dim];
  rbc_67_elt Fi_inv;
  uint32_t E_dim;

  rbc_67_vspace_init(&S, ec_size);
  rbc_67_vspace_set(S, ec, ec_size);
  rbc_67_vec_gauss(S, ec_size, NULL, 0);
  ...
  for(size_t i = 0 ; i < F_dim ; ++i) {
    rbc_67_vspace_init(&(Si[i]), E_expected_dim * F_dim);
    rbc_67_elt_inv(Fi_inv, F[i]);
    rbc_67_vec_scalar_mul(Si[i], S, Fi_inv, E_expected_dim * F_dim);
  }

  //Intersect every Si
  rbc_67_vspace_intersection(E, Si[0], Si[1], E_expected_dim * F_dim, E_expected_dim * F_dim);

  for(size_t i = 2 ; i < F_dim ; i++) {
    rbc_67_vspace_intersection(E, E, Si[i], E_expected_dim * F_dim, E_expected_dim * F_dim);
  }

  E_dim = rbc_67_vec_gauss(E, E_expected_dim * F_dim, NULL, 0);

  rbc_67_vspace_clear(S);
  for(size_t i = 0 ; i < F_dim ; ++i) {
    rbc_67_vspace_clear(Si[i]);
  }

  #ifdef VERBOSE
    printf("\n\n# Rank Support Recover - Begin #");
    printf("\n\nS: "); rbc_67_vec_print(S, E_expected_dim * F_dim);
    for(size_t i = 0 ; i < F_dim ; ++i) {
      printf("\n\nS%i: ", i); rbc_67_vec_print(Si[i], E_expected_dim * F_dim);
    }
    printf("\n\nE: "); rbc_67_vec_print(E, E_dim);
...
/**
 * @fn         unsigned int rbc_67_lrpc_RSR_vartime(rbc_67_vspace E, const uint32_t E_expected_dim, const rbc_67_vspace F, const uint32_t F_dim, const rbc_67_vec ec, const uint32_t ec_size)
 *
 * @brief      This function implements the rank_support_recover using
...
unsigned int rbc_67_lrpc_RSR_vartime(rbc_67_vspace E, const uint32_t E_expected_dim, const rbc_67_vspace F, const uint32_t F_dim, const rbc_67_vec ec, const uint32_t ec_size) {
  ...
  rbc_67_vspace_init(&S, ec_size);
  rbc_67_vspace_set(S, ec, ec_size);
  S_dim = rbc_67_vec_gauss_vartime(S, ec_size, NULL, 0);
  ...
  // Compute the spaces Si = F[i]^(-1) * S (line 10)
  for(size_t i = 0 ; i < F_dim ; ++i) {
    rbc_67_vspace_init(&(Si[i]), S_dim);
    rbc_67_elt_inv(Fi_inv, F[i]);
    rbc_67_vec_scalar_mul(Si[i], S, Fi_inv, S_dim);
  }

  //Intersect every Si,i+1
  //This supposes d >= 4
  E_dim = rbc_67_vspace_intersection_vartime(E, Si[0], Si[1], S_dim, S_dim);

  for(size_t i = 0 ; i < F_dim ; i++) {
    if(E_dim == E_expected_dim) break;
    E_dim = rbc_67_vspace_intersection_vartime(E, E, Si[i], E_dim, S_dim);
  }

  rbc_67_vec_echelonize(E, E_dim);
