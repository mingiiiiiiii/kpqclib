...
main()
{
    ...
    sprintf(fn_req, "PALOMA_%d.req", 64+64*flag);
    if ( (fp_req = fopen(fn_req, "w")) == NULL ) {
        printf("Couldn't open <%s> for write\n", fn_req);
        return KAT_FILE_OPEN_ERROR;
    }
    sprintf(fn_rsp, "PALOMA_%d.rsp", 64+64*flag);
    ...
    do {
        ...
        if ( (ret_val = crypto_PALOMA_keypair(pk, sk, &PALOMAParam)) != 0) {              //수정해야함.
        ...
        if ( (ret_val = crypto_PALOMA_enc(ct, ss, pk,&PALOMAParam)) != 0) {          //수정해야함.
        ...
        if ( (ret_val = crypto_PALOMA_dec(ss1, ct, sk,&PALOMAParam)) != 0) {             //수정해야함.
